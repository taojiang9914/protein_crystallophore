    ...
    if not no_mask:
        ...
        if 1:
            ...
            if conditions ==  ['ph']:
                ...
                n_bins = len(conditions)+1
            hit_counter = np.zeros([n_bins])
            for cond in condition_list:
                for c in cond:
                    hit_counter[c] += 1
            for i in range(n_bins):
                plt.text(xloc[i]-0.01, 0.4-vspace,str(int(hit_counter[i])),color='white')
...
for protein in proteins:
    ...
    for conditions in all_conditions:
        ...
        plot_scores(total_score_crys, conditions, condition_list,kit_names, protein, complexes, name_token, no_mask, show_plot)
